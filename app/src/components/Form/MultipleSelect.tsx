      ...
      render={({ field, fieldState }) => (
        <FormControl fullWidth error={Boolean(fieldState.error)}>
          ...
          <Select
            ...
            renderValue={(selected: string[]) => ( // Especificando explicitamente o tipo de 'selected' como string[]
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                {selected.map((value: string) => ( // Especificando explicitamente o tipo de 'value'
            ...
            MenuProps={MenuProps}
          >
            {list.map((name) => (  // Aqui você vai mapear a lista passada como props
              <MenuItem
                key={name}
                value={name}
